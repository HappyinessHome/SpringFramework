...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 创建BeanFactory对象 并解析xml中的信息转换为BeanDefinition对象 放入到BeanFactory中的BeanDefinitionMap中
			...
			try {
				...
				// 将place-holder的真实值注入到BeanDefinition中
				// 调用BeanFactoryPostProcessor各个实现类的postProcessBeanFactory(factory) 方法 即自定义的类实现BeanFactoryPostProcessor接口的
				invokeBeanFactoryPostProcessors(beanFactory);

				/**
				 * 注册所有的 BeanPostProcessor将所有实现了 BeanPostProcessor接口的类加载到 BeanFactory 中。
				 ...
				 * 具体的：在所有 bean 实例化时执行初始化方法前会调用所有BeanPostProcessor的postProcessBeforeInitialization 方法
				 * ，执行初始化方法后会调用所有 BeanPostProcessor 的 postProcessAfterInitialization
				 * 方法。
				 */
				...
				/**
				 * 实例化所有剩余的非懒加载单例 bean。
				 * 除了一些内部的 bean、实现了 BeanFactoryPostProcessor 接口的 bean、
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// （这个主要是对bean的属性设置管理的一个属性工具类，比如date的format，我们也可以进行自定义）
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// .添加默认的beanPostProcessor，此类用来完成某些aware对象的注入，我们后续会调用到。
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		//设置忽略自动装配的接口，这些接口的实现是容器是需要 beanPostProcessor 注入的，
		// 是要在后面进行统一处理的，所以要对他们进行忽略
